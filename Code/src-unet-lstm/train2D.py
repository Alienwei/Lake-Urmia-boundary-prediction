...
def train():
    ...
    #with tf.Graph().as_default(),tf.device('/cpu:0'):
        ...
        train_data_provider = datagenerator.load_file_lstm_1("/home/thesis/乌尔米湖/1",vol_size,1,17)
        train_x = train_data_provider[0]
        train_x = tf.cast(train_x,tf.float32)
        train_y = train_data_provider[1]
...
#        val_seg_measure = k.metrics.Mean(name='val_seg_measure')
        ...
        with tf.Session(config=config) as sess:   
            ...
            for step in range(total_step):
                train_val,total_loss_val,train_loss_val,acc,acc_op_val,flow_loss_val,flow_acc_val,flow_acc_op_val,y_val,flow_val = sess.run([train_op,total_loss,train_loss,train_accuracy,acc_op,flow_loss,flow_acc,flow_acc_op,y_pred,flow_pred])
                print(print_str.format(time.time() - start_time,step, total_loss_val,flow_loss_val,acc,flow_acc_op_val))
                #print(y_val.shape)
...
#            optimizer.apply_gradients(zip(gradients, model.trainable_variables))
            #print(flow_loss_val,total_loss)
                if (step) % 100 == 0:
                    ...
                    outputImg.save(os.path.join(model_dir, 'y_pred'+str(step)+'.png'))
                    ...
                    result = np.uint8(Gxy)
                    cv2.imwrite(os.path.join(model_dir,"flow_y"+str(step)+".png"), result)
                    outputImg2 = flow[0,:,:,1]
                    ...
                    B = max(Img2List)
                    if(A==B) and (A==0):
                        np.save(os.path.join(model_dir, 'flow_x'+str(step)+'.npy'), flow_val[0,:,:,0])
                    else:
                        Gxy = (255/(B-A))*abs(outputImg2-A)
                        result = np.uint8(Gxy)
                        cv2.imwrite(os.path.join(model_dir,"flow_x"+str(step)+".png"), result)
                #if (step) % 100 == 0:
