...
def load_file_lstm_1(image_dir,volsize,samples,time):
    ...
    res3.append(b[-1])
    return res3
def load_file_lstm_2(image_dir,volsize,samples,time):
    li=os.listdir(image_dir)
    os.chdir(image_dir)
    li.sort()
    b=[]
    ...
    for i in range(samples):
        ...
        for j in range(time-1):
            sample=np.concatenate((sample,b[i+j+1]),axis=0)
        sample = np.reshape(sample,(1,time,*volsize,1))
        res1.append(sample)
        res2.append(b[i+time])
        print(i+time)
    return res1,res2
...
def global_linear_transmation(image_dir): 
    li=os.listdir(image_dir)
    os.chdir(image_dir)
...
def flow(image_dir,filename):
    os.chdir(image_dir)
    img = np.array(Image.open(filename).convert('L'))
    reshapeGray = img.reshape(51200)
    reshapeGrayList = reshapeGray.tolist()
    A = min(reshapeGrayList)
    B = max(reshapeGrayList)
    print('A = %d,B = %d' % (A, B))
    Gxy = (255 / (B - A)) * abs(img- A)
    result = np.uint8(Gxy)
    print(result)
    cv2.imwrite("flow_x.jpg", result)
