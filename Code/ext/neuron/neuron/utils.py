...
#    TODO:
#        enable optional orig_grid - the original grid points.
#        check out tf.contrib.resampler, only 
#        
#        seems to work for 2D data
#    """
#    #print(loc.get_shape().as_list())
#    if isinstance(loc, (list, tuple)):
#        loc = tf.stack(loc,-1)#(512,512,2)
#        #print(loc.get_shape().as_list())
#    # since loc can be a list, nb_dims has to be based on vol.
#    #nb_dims = 2#loc.shape[-1]#
#    nb_dims = 2
#    #print(loc[0,...].shape)（2,512,512)
#    #vol.shape(512,512,1)
#    if nb_dims != len(vol.shape[:-1]):
#        raise Exception("Number of loc Tensors %d does not match volume dimension %d"
#                        % (nb_dims, len(vol.shape[:-1])))
...
#        volshape = vol.shape.as_list()#[512,512,1]
#    else:
#        volshape = vol.shape
#
#    # interpolate
#    if interp_method == 'linear':
#        loc0 = tf.floor(loc)#向下取整,找到整数点
#
#        # clip values
#        max_loc = [d - 1 for d in vol.get_shape().as_list()]
#        clipped_loc = [tf.clip_by_value(loc[...,d], 0, max_loc[d]) for d in range(nb_dims)]
...
#
#    # parse shapes
##    if isinstance(loc_shift.shape, (tf.Dimension, tf.TensorShape)):#loc_shift.shape=(512,512,2)
##        volshape = loc_shift.shape[:-1].as_list()#[512,512]
##        #print(loc_shift[...,0].shape,loc_shift.shape[0:-1])
##    else:
#    volshape = loc_shift.shape[:-1].as_list()
#    nb_dims = len(volshape)#2
#
#    # location should be mesh and delta
#    mesh = volshape_to_meshgrid(volshape, indexing=indexing)  # volume mesh[[(512,512)],[(512,512)]]
#    #print(mesh)
#    loc = [tf.cast(mesh[d], 'float32') + loc_shift[...,d] for d in range(nb_dims)]#形成flow场
#    #print(loc)
#    # test single
#    return interpn(vol, loc, interp_method=interp_method)
#
...
#        raise ValueError("volshape needs to be a list of integers")
#
#    linvec = [tf.range(0, d) for d in volshape]#[[0,2,3,4,5……511],[0,2,3,4,5……511]]
#    return meshgrid(*linvec, **kwargs)#[(512,512),(512,512)]一个列表，包含两个shape为（512，512）的数组
