...
#调用父类的init方法
    ...
    def build(self, input_shape):
        ...
        self.ndims = len(input_shape[0]) - 2#2
        self.inshape = input_shape
        vol_shape = input_shape[0][1:-1]#(512,512)
        trf_shape = input_shape[1][1:]#(512,512,2)

        # the transform is an affine if:
    ...
    def call(self, inputs):
        ...
        vol = inputs[0]#(none,512,512,1)
        trf = inputs[1]#(none,512,512,2)

        # necessary for multi_gpu models...
        vol = K.reshape(vol, [-1, *self.inshape[0][1:]])#(1,512,512,1)
        trf = K.reshape(trf, [-1, *self.inshape[1][1:]])#(1,512,512,2)
